...
a2a_assert_grep "qloop_question[[:space:]]+qloop_recipient[[:space:]]+qloop_words_avg[[:space:]]+cell_fragments[[:space:]]+cell_quality[[:space:]]+cell_tail[[:space:]]+cell_morph.*cell_question[[:space:]]+cell_words_avg" "$field_sweep_out" "field sweep reports cell surface quality and density columns"
...
a2a_assert_grep "^0[[:space:]]+0[[:space:]]+0\\.20[[:space:]]+0[[:space:]]+-0\\.10[[:space:]]+0\\.0[[:space:]]+0[[:space:]]+0[[:space:]]+0[[:space:]]+1[[:space:]]+2[[:space:]]+3[[:space:]]+1[[:space:]]+0[[:space:]]+0" "$field_grid_out" "field grid reports one no-qloop setting"
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 43 && row == 43) ? 0 : 1 }'; then
