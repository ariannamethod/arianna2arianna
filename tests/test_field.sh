...
a2a_assert_grep "^xcell[[:space:]]+qloop[[:space:]]+qloop_tconf_weight[[:space:]]+qloop_tconf_adapt[[:space:]]+qloop_tconf_adapt_weight[[:space:]]+rounds[[:space:]]+cells[[:space:]]+frag" "$field_grid_out" "field grid reports compact TSV header"
...
a2a_assert_grep "^0[[:space:]]+0[[:space:]]+0\\.20[[:space:]]+0[[:space:]]+-0\\.10[[:space:]]+1[[:space:]]+2[[:space:]]+3[[:space:]]+1[[:space:]]+0[[:space:]]+0" "$field_grid_out" "field grid reports one no-qloop setting"
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 38 && row == 38) ? 0 : 1 }'; then
