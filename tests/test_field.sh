...
a2a_assert_grep "qloop_question[[:space:]]+qloop_recipient[[:space:]]+qloop_words_avg[[:space:]]+cell_fragments[[:space:]]+cell_quality[[:space:]]+cell_tail[[:space:]]+cell_morph.*cell_question[[:space:]]+cell_words_avg[[:space:]]+base_ms[[:space:]]+base_gen[[:space:]]+base_retry[[:space:]]+base_probe[[:space:]]+qloop_ms[[:space:]]+qloop_gen[[:space:]]+qloop_retry[[:space:]]+elapsed_sec" "$field_sweep_out" "field sweep reports cell surface quality, phase timing, and latency columns"
...
if printf "%s\n" "$field_sweep_out" | awk -F '\t' '$1 == "Let the cells remember each other." && $16 ~ /^[0-9]+$/ && $17 ~ /^[0-9]+$/ && $18 ~ /^[0-9]+$/ && $19 ~ /^[0-9]+$/ && $20 ~ /^[0-9]+$/ && $21 ~ /^[0-9]+$/ && ($22 == "nan" || $22 ~ /^[0-9]+[.][0-9]+$/) && ($23 == "nan" || $23 ~ /^[0-9]+[.][0-9]+$/) && ($24 == "nan" || $24 ~ /^[0-9]+[.][0-9]+$/) && ($25 == "nan" || $25 ~ /^[0-9]+[.][0-9]+$/) && ($26 == "nan" || $26 ~ /^[0-9]+[.][0-9]+$/) && ($27 == "nan" || $27 ~ /^[0-9]+[.][0-9]+$/) && ($28 == "nan" || $28 ~ /^[0-9]+[.][0-9]+$/) && ($29 == "nan" || $29 ~ /^[0-9]+[.][0-9]+$/) && ($30 == "nan" || $30 ~ /^[0-9]+[.][0-9]+$/) && ($31 == "nan" || $31 ~ /^[0-9]+[.][0-9]+$/) && ($32 == "nan" || $32 ~ /^[+-]?[0-9]+[.][0-9]+$/) && $33 ~ /^[0-9]+$/ && $34 ~ /^[0-9]+$/ && $35 ~ /^[0-9]+$/ && $36 ~ /^[0-9]+$/ && $37 ~ /^[0-9]+$/ && $38 ~ /^[0-9]+$/ && $44 ~ /^[0-9]+$/ && ($45 == "nan" || $45 ~ /^[0-9]+[.][0-9]+$/) && $46 == 6 && $47 ~ /^[0-9]+$/ && ($53 == "nan" || $53 ~ /^[0-9]+[.][0-9]+$/) && ($54 == "nan" || $54 ~ /^[0-9]+$/) && $55 ~ /^[0-9]+$/ && $56 ~ /^[0-9]+$/ && $57 ~ /^[0-9]+$/ && ($58 == "nan" || $58 ~ /^[0-9]+$/) && $59 ~ /^[0-9]+$/ && $60 ~ /^[0-9]+$/ && $61 ~ /^[0-9]+$/ { ok = 1 } END { exit ok ? 0 : 1 }'; then
...
a2a_assert_grep "^xcell[[:space:]]+qloop[[:space:]]+qloop_tconf_weight[[:space:]]+qloop_tconf_adapt[[:space:]]+qloop_tconf_adapt_weight[[:space:]]+qloop_min_iq[[:space:]]+qloop_unique_asker[[:space:]]+qloop_candidate_pool[[:space:]]+qloop_statement_pool[[:space:]]+qloop_statement_routes[[:space:]]+cell_retry_max[[:space:]]+rounds[[:space:]]+cells[[:space:]]+frag" "$field_grid_out" "field grid reports compact TSV header"
a2a_assert_grep "qloop_gated.*qloop_stmt_routes.*qloop_stmt_gated.*qloop_efficiency.*qloop_score_avg.*qloop_gate_score_avg.*qloop_profile.*qloop_gate_profile.*qloop_words_avg.*cell_words_avg.*i_n_signs[[:space:]]+avg_i_n_kv[[:space:]]+i_q_signs[[:space:]]+i_q_bands[[:space:]]+avg_i_q_kv.*d_margin_signs.*field_score[[:space:]]+base_ms_avg[[:space:]]+base_ms_max[[:space:]]+base_gen[[:space:]]+base_retry[[:space:]]+base_probe[[:space:]]+qloop_ms_avg[[:space:]]+qloop_ms_max[[:space:]]+qloop_gen[[:space:]]+qloop_retry[[:space:]]+elapsed_avg[[:space:]]+elapsed_max[[:space:]]+raw_dir" "$field_grid_out" "field grid reports influence, density, settling risks, score, phase timing, latency, and raw columns"
a2a_assert_grep "^0[[:space:]]+0[[:space:]]+0\\.20[[:space:]]+0[[:space:]]+-0\\.10[[:space:]]+0\\.0[[:space:]]+0[[:space:]]+0[[:space:]]+0[[:space:]]+0[[:space:]]+4[[:space:]]+1[[:space:]]+2[[:space:]]+3[[:space:]]+1[[:space:]]+0[[:space:]]+0" "$field_grid_out" "field grid reports one no-qloop setting"
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 58 && row == 58) ? 0 : 1 }'; then
