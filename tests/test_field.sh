...
a2a_assert_grep "^xcell[[:space:]]+qloop[[:space:]]+qloop_tconf_weight[[:space:]]+qloop_tconf_adapt[[:space:]]+qloop_tconf_adapt_weight[[:space:]]+qloop_min_iq[[:space:]]+qloop_unique_asker[[:space:]]+qloop_candidate_pool[[:space:]]+rounds[[:space:]]+cells[[:space:]]+frag" "$field_grid_out" "field grid reports compact TSV header"
a2a_assert_grep "qloop_gated.*qloop_efficiency.*qloop_score_avg.*qloop_gate_score_avg.*qloop_profile.*qloop_gate_profile.*qloop_words_avg.*cell_words_avg.*i_n_signs[[:space:]]+avg_i_n_kv[[:space:]]+i_q_signs[[:space:]]+i_q_bands[[:space:]]+avg_i_q_kv.*d_margin_signs.*field_score[[:space:]]+raw_dir" "$field_grid_out" "field grid reports influence, density, settling risks, score, and raw columns"
a2a_assert_grep "^0[[:space:]]+0[[:space:]]+0\\.20[[:space:]]+0[[:space:]]+-0\\.10[[:space:]]+0\\.0[[:space:]]+0[[:space:]]+0[[:space:]]+1[[:space:]]+2[[:space:]]+3[[:space:]]+1[[:space:]]+0[[:space:]]+0" "$field_grid_out" "field grid reports one no-qloop setting"
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 42 && row == 42) ? 0 : 1 }'; then
