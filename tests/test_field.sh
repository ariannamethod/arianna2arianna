...
a2a_assert_grep "qloop_triggers[[:space:]]+qloop_gated[[:space:]]+qloop_score_avg[[:space:]]+qloop_gate_score_avg[[:space:]]+qloop_iq_avg[[:space:]]+qloop_iq_pos[[:space:]]+qloop_iq_neg[[:space:]]+qloop_iq_zero[[:space:]]+qloop_quality" "$field_sweep_out" "field sweep reports qloop influence sign columns"
...
if printf "%s\n" "$field_sweep_out" | awk -F '\t' '$1 == "Let the cells remember each other." && $16 ~ /^[0-9]+$/ && $17 ~ /^[0-9]+$/ && $18 ~ /^[0-9]+$/ && $19 ~ /^[0-9]+$/ && ($20 == "nan" || $20 ~ /^[0-9]+[.][0-9]+$/) && ($21 == "nan" || $21 ~ /^[0-9]+[.][0-9]+$/) && ($22 == "nan" || $22 ~ /^[+-]?[0-9]+[.][0-9]+$/) && $23 ~ /^[0-9]+$/ && $24 ~ /^[0-9]+$/ && $25 ~ /^[0-9]+$/ && $26 ~ /^[0-9]+$/ && $32 == 6 && $33 ~ /^[0-9]+$/ { ok = 1 } END { exit ok ? 0 : 1 }'; then
...
a2a_assert_grep "qloop_gated.*qloop_efficiency.*qloop_score_avg.*qloop_gate_score_avg.*i_n_signs[[:space:]]+avg_i_n_kv[[:space:]]+i_q_signs[[:space:]]+avg_i_q_kv.*d_margin_signs.*field_score[[:space:]]+raw_dir" "$field_grid_out" "field grid reports influence and settling risks, score, and raw columns"
...
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 31 && row == 31) ? 0 : 1 }'; then
