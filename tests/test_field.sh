...
a2a_assert_grep "qloop_gated.*qloop_efficiency.*qloop_score_avg.*qloop_gate_score_avg.*qloop_profile.*qloop_gate_profile.*i_n_signs[[:space:]]+avg_i_n_kv[[:space:]]+i_q_signs[[:space:]]+avg_i_q_kv.*d_margin_signs.*field_score[[:space:]]+raw_dir" "$field_grid_out" "field grid reports influence and settling risks, score, and raw columns"
...
if printf "%s\n" "$field_grid_out" | awk -F '\t' 'NR == 1 && $1 == "xcell" { header = NF } NR > 1 && $1 == "0" { row = NF } END { exit (header == 35 && row == 35) ? 0 : 1 }'; then
