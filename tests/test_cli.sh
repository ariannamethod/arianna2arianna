...
a2a_assert_grep "A2A_TEMP_USER_TOKENS" "$out" "repl temp sweep help names answer token grid"
...
out="$(bash "$A2A_ROOT/tools/repl_dialogue_compare.sh" --help 2>&1)"
a2a_assert_grep "A2A_DIALOGUE_CANDIDATE_QLOOP" "$out" "dialogue compare help names candidate qloop"
a2a_assert_grep "A2A_DIALOGUE_BASE_QLOOP" "$out" "dialogue compare help names baseline qloop"
...
a2a_assert_grep "A2A_FIELD_QLOOP_TCONF_ADAPT_WEIGHTS" "$out" "field grid help names qloop adaptive target-confidence weight grid"
...
a2a_assert_grep "A2A_FIELD_PROMPT_FORMATS" "$out" "field grid help names field prompt format grid"
a2a_assert_grep "raw, qa, auto, user_arianna" "$out" "field grid help names prompt format choices"
a2a_assert_grep "A2A_FIELD_TEMP_BASES" "$out" "field grid help names field temperature base grid"
a2a_assert_grep "A2A_FIELD_TEMP_SPANS" "$out" "field grid help names field temperature span grid"
a2a_assert_grep "A2A_FIELD_LANG_BIASES" "$out" "field grid help names language bias grid"
...
printf "alpha\tsem\t2\t3\t1\t3.000\t0.500\t0.400\t+0.100\t+0.000\t0.000\t+0.000\t+0.200\t0.300\t0.40\t2\t2\t0\t0\t1\t0\t0.600\tnan\t0.400\tnan\t0.500\tnan\t0.300\tnan\t1.000\tnan\t+0.500\t2\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t4.000\t2\t0\t0\t0\t0\t0\t1\t0\t5.000\t100\t4\t1\t0\t1\t0\t40\t2\t1\t3\n" >> "$field_summary_tsv"
printf "beta\tsem\t2\t3\t1\t3.200\t0.600\t0.500\t+0.100\t+0.000\t0.000\t+0.000\t-0.100\t0.500\t0.60\t0\t0\t0\t0\t0\t0\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\t0\tnan\t2\t1\t1\t0\t0\t0\t0\t0\t3.500\t200\t2\t0\t0\t0\t0\t0\t0\t0\t5\n" >> "$field_summary_tsv"
...
a2a_assert_grep "qloop_statement: accepted 1/2, gated 0/0" "$field_summary_out" "field TSV summary reports statement-route split"
...
a2a_assert_grep "qloop_quality: any 0/2.*recipient 0, lang_mismatch 0" "$field_summary_out" "field TSV summary reports qloop debt"
a2a_assert_grep "cell_quality: any 1/4, tail 1.*lang_mismatch 0" "$field_summary_out" "field TSV summary reports cell debt"
a2a_assert_grep "density: qloop_words_avg 4\\.000, cell_words_avg 4\\.250" "$field_summary_out" "field TSV summary reports answer density"
a2a_assert_grep "I_N\\^kv: avg \\+0\\.050" "$field_summary_out" "field TSV summary reports neighbour influence average"
a2a_assert_grep "I_Q\\^kv: avg \\+0\\.500, pos 2, neg 0, zero 0, low 0, strong 0" "$field_summary_out" "field TSV summary reports qloop influence signs and bands"
...
a2a_assert_grep "timing: base_ms_avg 150, base_ms_max 200 :: beta, base_gen 6, base_retry 1, base_probe 0, base_rescue 1, base_fail 0, qloop_ms_avg 20, qloop_ms_max 40 :: alpha, qloop_gen 2, qloop_retry 1" "$field_summary_out" "field TSV summary reports phase timing"
a2a_assert_grep "latency: avg_sec 4\\.000, max_sec 5\\.000 :: beta" "$field_summary_out" "field TSV summary reports latency"
...
rm -f "$junk_tsv"
a2a_assert_grep "answer_quality: any 8/8, short 0, question_like 0, label_artifact 0, notation_artifact 0, morph_artifact 6, recipient_artifact 2, tail_artifact 6" "$junk_out" "repl TSV summary flags domain, assignment, recipient, contraction, sparse Cyrillic, and tail junk"
